...
近期我们收到大量反馈，存在部分第三方账号以“MAA 教程 / 整合包 / 拓展功能”等名义发布视频，并通过“关注 + 三连 + 私信 / 网盘”等方式传播资源。  
目前已发现有人冒充项目创建假仓库、篡改源码署名等行为，甚至在其中夹带恶意可执行文件。  
这些恶意程序会检测虚拟机、调试环境，一旦发现便自动退出或销毁，以躲避分析，极为隐蔽。  
我们已经针对已发现的仿冒仓库提交了 DMCA 投诉，但由于处理流程原因，相关仓库目前仍可能被搜索到，请勿自行在 GitHub 上搜索下载。

在此提醒大家：

• MAA 及相关项目完全免费、开源，任何要求，不会以“关注、三连、私信”等方式作为获取资源的前提条件；  
• 官方唯一安全入口是官网。正确的 GitHub 仓库链接只通过官网提供，请勿在任何搜索引擎或 GitHub 站内直接搜索项目名来获取资源；  
• 因部分地区 GitHub 下载较慢，我们会在官方用户群的群文件中同步上传最新版压缩包。唯一官方群入口也在官网内公示，请不要通过其他任何渠道加群；  
...
• **软件版本**：解压完整包至**新**文件夹，可复制旧文件夹里的 `config` 和 `data` 文件夹来保留数据  
  ...
  🆕 v6.8.0-beta.2 及之后版本支持将软件完整包或对应版本的 OTA 更新 zip 包直接拖入软件窗口自动更新，并会正常执行旧文件清理步骤  
